...
import type {
  ScoreValidationContext,
  ScoreValidationSummary,
} from './score-validation.types';
...
export function validateFootballScoreDetails(
      ...
      ? (() => {
          ...
          if (
            ...
            throw new BadRequestException(
              'football.team1Goals và football.team2Goals phải là số nguyên không âm.',
            );
          }
          const phase =
            typeof value.phase === 'string' ? value.phase : 'FIRST_HALF';
          ...
          if (
            ...
            throw new BadRequestException(
              'football.minute không hợp lệ (0-150).',
            );
          }
          if (
            value.addedMinute !== undefined &&
            (!Number.isInteger(value.addedMinute) ||
              Number(value.addedMinute) < 0 ||
              Number(value.addedMinute) > 30)
          ) {
            throw new BadRequestException(
              'football.addedMinute không hợp lệ (0-30).',
            );
          }
          if (value.events !== undefined) {
            if (!Array.isArray(value.events) || value.events.length > 500) {
              throw new BadRequestException(
                'football.events phải là danh sách tối đa 500 sự kiện.',
              );
            }
            for (const event of value.events) {
              if (!event || typeof event !== 'object' || Array.isArray(event)) {
                throw new BadRequestException(
                  'football.events chứa phần tử không hợp lệ.',
                );
              }
              ...
              if (
                ...
                throw new BadRequestException(
                  'football.events.type không hợp lệ.',
                );
              }
              if (item.team !== 1 && item.team !== 2) {
                throw new BadRequestException(
                  'football.events.team phải là 1 hoặc 2.',
                );
              }
              if (
                ...
                throw new BadRequestException(
                  'football.events.minute không hợp lệ.',
                );
              }
              if (
                item.addedMinute !== undefined &&
                (!Number.isInteger(item.addedMinute) ||
                  Number(item.addedMinute) < 0 ||
                  Number(item.addedMinute) > 30)
              ) {
                throw new BadRequestException(
                  'football.events.addedMinute không hợp lệ.',
                );
              }
            }
          }
  ...
  for (const [index, entry] of normalizedEntries.entries()) {
    const isLiveFinalSet =
      index === lastEntryIndex && entry.isFinished === false;
    if (!entry.isFinished && !isLiveFinalSet) {
      throw new BadRequestException(
        `Trận ${entry.key}: Chỉ trận đang diễn ra cuối cùng mới được để trạng thái chưa kết thúc.`,
      );
    }
    ...
    if (p1 < 0 || p2 < 0) {
      throw new BadRequestException(
        `Trận ${entry.key}: Tỷ số bàn thắng không được âm.`,
      );
    }
    ...
    if (
      ...
      throw new BadRequestException(
        `Không được nhập thêm ${entry.key} sau khi trận đã chốt người thắng từ trước.`,
      );
    }
  }
