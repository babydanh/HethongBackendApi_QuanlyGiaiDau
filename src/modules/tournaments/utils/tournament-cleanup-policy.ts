export const TOURNAMENT_CLEANUP_GRACE_DAYS = 7;
...
export type TournamentCleanupReason =
  | 'NO_ACTIVE_REGISTRATION_AFTER_GRACE_PERIOD'
  | 'STATUS_NOT_ELIGIBLE'
  | 'REGISTRATION_END_DATE_MISSING'
  | 'REGISTRATION_END_DATE_INVALID'
  | 'REGISTRATION_START_DATE_INVALID'
  | 'REGISTRATION_WINDOW_INVALID'
  | 'REGISTRATION_NOT_EXPIRED'
...
export function evaluateTournamentCleanup(
  snapshot: TournamentCleanupSnapshot,
  now: Date,
): TournamentCleanupDecision {
  ...
  // A future event must never be auto-cancelled merely because registration
  // ended. The organizer still needs the tournament for extension/management.
  if (startDate > now) {
  ...
  if (
    !Number.isFinite(activeRegistrationCount) ||
  ...
  gracePeriodEndsAt.setUTCDate(
    gracePeriodEndsAt.getUTCDate() + TOURNAMENT_CLEANUP_GRACE_DAYS,
  );
...
export function isProtectedRegistrationStatus(status: string | null | undefined): boolean {
