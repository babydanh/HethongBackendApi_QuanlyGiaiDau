...
export const communitySocialSettings = pgTable('community_social_settings', {
  ...
  postingPolicy: varchar('posting_policy', { length: 30 })
    .default('MEMBERS')
    .notNull(),
...
export const communityPosts = pgTable(
  'community_posts',
  {
    ...
    clubMatchSessionId: uuid('club_match_session_id').references(
      () => clubMatchSessions.id,
      { onDelete: 'cascade' },
    ),
  ...
  (table) => ({
    feedIndex: index('idx_community_posts_feed').on(
      table.communityId,
      table.createdAt,
      table.id,
    ),
    ...
    tournamentIndex: index('idx_community_posts_tournament').on(
      table.tournamentId,
    ),
    clubMatchSessionIndex: index('idx_community_posts_club_match_session').on(
      table.clubMatchSessionId,
    ),
...
export const communityPostComments = pgTable(
  ...
  (table) => ({
    postIndex: index('idx_community_post_comments_post').on(
      table.postId,
      table.createdAt,
      table.id,
    ),
...
export const communityPostReactions = pgTable(
  ...
  (table) => ({
    uniqueReaction: uniqueIndex('uq_community_post_reactions_user').on(
      table.postId,
      table.userId,
    ),
  }),
...
export const communitySocialReports = pgTable(
  'community_social_reports',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    communityId: uuid('community_id')
      .references(() => communities.id, { onDelete: 'cascade' })
    ...
    commentId: uuid('comment_id').references(() => communityPostComments.id, {
      onDelete: 'cascade',
    }),
  ...
  (table) => ({
    queueIndex: index('idx_community_social_reports_queue').on(
      table.communityId,
      table.status,
      table.createdAt,
    ),
...
export const communityMemberSocialPreferences = pgTable(
  'community_member_social_preferences',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    communityId: uuid('community_id')
      .references(() => communities.id, { onDelete: 'cascade' })
  ...
  (table) => ({
    uniqueMemberPreference: uniqueIndex(
      'uq_community_member_social_preferences',
...
export const communityPolls = pgTable(
  'community_polls',
  {
    ...
    allowMultipleAnswers: boolean('allow_multiple_answers')
      .default(false)
      .notNull(),
  ...
  (table) => ({
    ...
    communityPollIndex: index('idx_community_polls_community').on(
      table.communityId,
    ),
  }),
);
...
export const communityPollOptions = pgTable(
  'community_poll_options',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    pollId: uuid('poll_id')
      .references(() => communityPolls.id, { onDelete: 'cascade' })
      .notNull(),
    creatorId: uuid('creator_id').references(() => users.id, {
      onDelete: 'set null',
    }),
...
export const communityPollVotes = pgTable(
  'community_poll_votes',
  {
    ...
    optionId: uuid('option_id')
      .references(() => communityPollOptions.id, { onDelete: 'cascade' })
      .notNull(),
    userId: uuid('user_id')
      .references(() => users.id, { onDelete: 'cascade' })
      .notNull(),
  ...
  (table) => ({
    uniqueVote: uniqueIndex('uq_community_poll_votes_user_option').on(
      table.optionId,
      table.userId,
    ),
    pollVoteIndex: index('idx_community_poll_votes_poll').on(
      table.pollId,
      table.userId,
    ),
  }),
