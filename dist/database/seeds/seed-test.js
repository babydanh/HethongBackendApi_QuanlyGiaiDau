...
async function simulateMatches(tournamentId, authHeaders) {
    ...
    while (hasPlayableMatches && iteration < 20) {
        ...
        if (playable.length === 1 && matches.filter((m) => m.status !== 'COMPLETED').length === 1) {
            break;
        }
    ...
    for (let i = 1; i <= numTeams; i++) {
        if (isDoubles) {
            ...
            await db.insert(schema.userRanks).values({
                ...
                categoryId: categoryId,
                matchType: matchType,
                eloPoints: Math.floor(Math.random() * (1600 - 1000 + 1) + 1000),
                matchesPlayed: 10,
                matchesWon: 6,
            });
...
async function main() {
    ...
    const oldTours = await db.select().from(schema.tournaments).where((0, drizzle_orm_1.sql) `invite_code LIKE 'T-%'`);
    ...
    const organizer = await db.select().from(schema.users).where((0, drizzle_orm_1.eq)(schema.users.email, 'organizer@vndcsport.com')).limit(1).then(r => r[0]);
    if (!organizer) {
    ...
    const configs = [
        { name: '1. Giải loại trực tiếp 10 đội', bracketType: 'single_elimination', numTeams: 10, matchType: 'SINGLES' },
        { name: '2. Giải loại trực tiếp 11 đội', bracketType: 'single_elimination', numTeams: 11, matchType: 'SINGLES' },
        { name: '3. Giải loại trực tiếp 12 đội (Đôi)', bracketType: 'single_elimination', numTeams: 12, matchType: 'DOUBLES' },
        { name: '4. Giải loại trực tiếp 13 đội', bracketType: 'single_elimination', numTeams: 13, matchType: 'SINGLES' },
        { name: '5. Giải thắng/thua 10 đội', bracketType: 'double_elimination', numTeams: 10, matchType: 'SINGLES' },
        { name: '6. Giải thắng/thua 11 đội (Đôi)', bracketType: 'double_elimination', numTeams: 11, matchType: 'DOUBLES' },
        { name: '7. Giải thắng/thua 12 đội', bracketType: 'double_elimination', numTeams: 12, matchType: 'SINGLES' },
        { name: '8. Giải thắng/thua 13 đội (Đôi)', bracketType: 'double_elimination', numTeams: 13, matchType: 'DOUBLES' },
        { name: '9. Giải vòng tròn 6 đội', bracketType: 'round_robin', numTeams: 6, matchType: 'SINGLES' },
        { name: '10. Giải vòng tròn 7 đội (Đôi)', bracketType: 'round_robin', numTeams: 7, matchType: 'DOUBLES' },
        { name: '11. Giải vòng tròn 8 đội (Max)', bracketType: 'round_robin', numTeams: 8, matchType: 'SINGLES' },
        {
    ...
    for (const cfg of configs) {
        ...
        if (cfg.bracketType === 'group_stage_knockout') {
            const stage = await db.select().from(schema.tournamentStages).where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema.tournamentStages.tournamentId, tour.tourId), (0, drizzle_orm_1.eq)(schema.tournamentStages.type, 'GROUP_STAGE'))).limit(1).then(r => r[0]);
            if (stage) {
