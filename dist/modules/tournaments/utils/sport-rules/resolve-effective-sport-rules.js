...
exports.resolveEffectiveSportRules = resolveEffectiveSportRules;
...
function inferKindFromCategoryName(categorySlug, categoryName) {
...
function getRoundOverride(config, roundNumber) {
...
function getScoringView(source) {
    if (!source) {
        return null;
    }
    return {
...
function readNumber(sources, keys) {
...
function resolveRuleKind(input, overrides) {
    ...
    const compatibleTournamentKind = areKindsCompatible(categoryKind, tournamentKind)
        ? tournamentKind
        : undefined;
    return (readExplicitKind(overrides.matchOverride) ||
...
function resolveEffectiveSportRules(input) {
    const tournamentRules = asRecord(input.tournamentSportRules);
    const categoryConfig = asRecord(input.categoryConfig);
    const categoryDefaults = getNestedRecord(categoryConfig, 'defaultSportRules');
    ...
    const pointsPerSet = Math.max(1, Math.trunc(readNumber(scoringSources, [
        'pointsPerSet',
        'points_per_set',
        'pointsPerGame',
        'points_per_game',
    ...
    const maxPoints = Math.max(pointsPerSet, Math.trunc(readNumber(scoringSources, ['maxPoints', 'max_points', 'maxPointsPerSet', 'capAt']) ?? defaults.maxPoints));
    const tiebreakAt = Math.max(0, Math.trunc(readNumber(scoringSources, ['tiebreakAt', 'tiebreak_at']) ?? defaults.tiebreakAt));
    ...
    const version = Math.max(1, Math.trunc(readNumber([effectiveTournamentRules], ['version']) ?? 1));
    return {
        version,
        kind,
...
//# sourceMappingURL=resolve-effective-sport-rules.js.map