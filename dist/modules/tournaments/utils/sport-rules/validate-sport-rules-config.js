...
exports.validateSportRuleConfig = validateSportRuleConfig;
exports.inferExpectedSportRuleKind = inferExpectedSportRuleKind;
exports.inferAllowedSportRuleKinds = inferAllowedSportRuleKinds;
const common_1 = require("@nestjs/common");
const sport_rule_kind_type_1 = require("./sport-rule-kind.type");
...
function normalizeSportRuleKind(raw) {
    ...
    return sport_rule_kind_type_1.SPORT_RULE_KINDS.includes(normalized)
        ? normalized
        : null;
}
...
function assertPositiveInteger(source, keys, sourceLabel, fieldLabel) {
    const value = readNumber(source, keys);
    if (value == null) {
        return;
    }
    if (!Number.isInteger(value) || value < 1) {
        throw new common_1.BadRequestException(`${sourceLabel}: ${fieldLabel} phải là số nguyên dương.`);
    }
}
function inferSportRuleKindFromCategory(source) {
...
function resolveAllowedKindsFromCategory(source) {
    const categoryConfig = asRecord(source.categoryConfig);
...
function validateFormatBlock(kind, format, sourceLabel) {
    const allowedKeys = FORMAT_KEYS[kind];
    for (const key of Object.keys(format)) {
        if (!allowedKeys.has(key)) {
            throw new common_1.BadRequestException(`${sourceLabel}: field format.${key} không hợp lệ cho luật ${kind}.`);
        }
    }
    switch (kind) {
        case 'TENNIS': {
            ...
            if (gamesPerSet != null && ![4, 6].includes(gamesPerSet)) {
                throw new common_1.BadRequestException(`${sourceLabel}: tennis chỉ hỗ trợ gamesPerSet là 4 hoặc 6.`);
            }
            const tiebreakAtGames = readNumber(format, ['tiebreakAtGames']);
            if (tiebreakAtGames != null && ![4, 6].includes(tiebreakAtGames)) {
                throw new common_1.BadRequestException(`${sourceLabel}: tennis chỉ hỗ trợ tiebreakAtGames là 4 hoặc 6.`);
            }
            const tiebreakPoints = readNumber(format, ['tiebreakPoints']);
            if (tiebreakPoints != null && ![7, 10].includes(tiebreakPoints)) {
                throw new common_1.BadRequestException(`${sourceLabel}: tennis chỉ hỗ trợ tie-break 7 hoặc 10 điểm.`);
            }
            const finalSetMode = format.finalSetMode;
            if (finalSetMode !== undefined &&
                finalSetMode !== 'STANDARD' &&
                finalSetMode !== 'SUPER_TIEBREAK' &&
                finalSetMode !== 'NO_TIEBREAK') {
                throw new common_1.BadRequestException(`${sourceLabel}: finalSetMode của tennis không hợp lệ.`);
            }
            const tiebreakMode = format.tiebreakMode;
            if (tiebreakMode !== undefined &&
                tiebreakMode !== 'STANDARD_7' &&
                tiebreakMode !== 'SUPER_10' &&
                tiebreakMode !== 'NONE') {
                throw new common_1.BadRequestException(`${sourceLabel}: tiebreakMode của tennis không hợp lệ.`);
            }
            break;
        }
        case 'PICKLEBALL_SIDE_OUT': {
            ...
            if (gamePoint != null && ![11, 15, 21].includes(gamePoint)) {
                throw new common_1.BadRequestException(`${sourceLabel}: pickleball side-out chỉ hỗ trợ gamePoint 11, 15 hoặc 21.`);
            }
            const firstServerRule = format.firstServerRule;
            if (firstServerRule !== undefined && firstServerRule !== 'STANDARD') {
                throw new common_1.BadRequestException(`${sourceLabel}: firstServerRule của side-out hiện chỉ hỗ trợ STANDARD.`);
            }
            const doublesServeFlow = format.doublesServeFlow;
            if (doublesServeFlow !== undefined && doublesServeFlow !== 'TWO_SERVER') {
                throw new common_1.BadRequestException(`${sourceLabel}: doublesServeFlow của side-out hiện chỉ hỗ trợ TWO_SERVER.`);
            }
            break;
        }
        default: {
            ...
            if (pointsPerGame != null && pointsPerGame < 1) {
                throw new common_1.BadRequestException(`${sourceLabel}: pointsPerGame phải là số nguyên dương.`);
            }
            const capAt = readNumber(format, ['capAt']);
            if (capAt != null && pointsPerGame != null && capAt < pointsPerGame) {
                throw new common_1.BadRequestException(`${sourceLabel}: capAt không được nhỏ hơn pointsPerGame.`);
            }
            break;
        }
    }
}
function validateScoringSemantics(kind, source, sourceLabel) {
    assertPositiveInteger(source, ['setsToWin', 'sets_to_win', 'bestOf', 'best_of'], sourceLabel, 'số set/game thắng');
    assertPositiveInteger(source, ['pointsPerSet', 'points_per_set', 'pointsPerGame', 'points_per_game', 'gamePoint', 'game_point'], sourceLabel, 'mốc điểm');
    assertPositiveInteger(source, ['tiebreakAt', 'tiebreak_at', 'tiebreakPoints', 'tiebreak_points', 'maxPoints', 'max_points', 'maxPointsPerSet'], sourceLabel, 'ngưỡng tie-break / điểm trần');
    ...
    if (pointsPerSet != null && maxPoints != null && maxPoints < pointsPerSet) {
        throw new common_1.BadRequestException(`${sourceLabel}: maxPoints không được nhỏ hơn pointsPerSet.`);
    }
    if (tiebreakAt != null && maxPoints != null && maxPoints > 0 && tiebreakAt >= maxPoints) {
        throw new common_1.BadRequestException(`${sourceLabel}: tiebreakAt phải nhỏ hơn maxPoints.`);
    }
    switch (kind) {
        case 'BADMINTON':
            if (pointsPerSet != null && pointsPerSet > 30) {
                throw new common_1.BadRequestException(`${sourceLabel}: cầu lông không hợp lệ nếu pointsPerSet vượt quá 30.`);
            }
            break;
        case 'TABLE_TENNIS':
            if (pointsPerSet != null && pointsPerSet > 99) {
                throw new common_1.BadRequestException(`${sourceLabel}: bóng bàn không hợp lệ nếu pointsPerSet vượt quá 99.`);
            }
            break;
        case 'PICKLEBALL_RALLY':
            if (pointsPerSet != null && ![11, 15, 21].includes(pointsPerSet)) {
                throw new common_1.BadRequestException(`${sourceLabel}: pickleball rally chỉ hỗ trợ pointsPerSet 11, 15 hoặc 21.`);
            }
            break;
        case 'PICKLEBALL_SIDE_OUT':
            if (pointsPerSet != null && ![11, 15, 21].includes(pointsPerSet)) {
                throw new common_1.BadRequestException(`${sourceLabel}: pickleball side-out chỉ hỗ trợ pointsPerSet 11, 15 hoặc 21.`);
            }
            if (winByTwo === false) {
                throw new common_1.BadRequestException(`${sourceLabel}: pickleball side-out phải giữ luật thắng cách 2.`);
            }
            break;
        case 'TENNIS':
            if (pointsPerSet != null && ![4, 6].includes(pointsPerSet)) {
                throw new common_1.BadRequestException(`${sourceLabel}: tennis chỉ hỗ trợ pointsPerSet/gamesPerSet là 4 hoặc 6.`);
            }
            if (maxPoints != null && pointsPerSet != null && ![pointsPerSet + 1, pointsPerSet + 2].includes(maxPoints)) {
                throw new common_1.BadRequestException(`${sourceLabel}: tennis chỉ cho phép maxPoints theo logic 1-2 game vượt chuẩn.`);
            }
            if (tiebreakPoints != null && ![7, 10].includes(tiebreakPoints)) {
                throw new common_1.BadRequestException(`${sourceLabel}: tennis chỉ hỗ trợ tie-break 7 hoặc 10 điểm.`);
            }
...
function validateScoringBlock(block, kind, sourceLabel) {
    ...
    for (const key of Object.keys(block)) {
        if (!allowedKeys.has(key)) {
            throw new common_1.BadRequestException(`${sourceLabel}: field scoring.${key} không hợp lệ cho luật ${kind}.`);
        }
    }
    if (kind === 'FOOTBALL' && block.forfeitGoals !== undefined) {
        ...
        if (typeof value !== 'number' ||
            ...
            value < 1 ||
            value > 99) {
            throw new common_1.BadRequestException(`${sourceLabel}: forfeitGoals phải là số nguyên từ 1 đến 99.`);
        }
    }
    validateScoringSemantics(kind, block, sourceLabel);
}
function validateGroupStageStructure(payload, sourceLabel) {
    const groupsConfig = asRecord(payload.groupsConfig);
    if (payload.groupsConfig !== undefined && !groupsConfig) {
        throw new common_1.BadRequestException(`${sourceLabel}: groupsConfig phải là object.`);
    }
    if (groupsConfig) {
        ...
        if (numGroups != null && numGroups < 2) {
            throw new common_1.BadRequestException(`${sourceLabel}: vòng bảng + loại trực tiếp phải có ít nhất 2 bảng.`);
        }
        if (teamsPerGroup != null && teamsPerGroup < 2) {
            throw new common_1.BadRequestException(`${sourceLabel}: mỗi bảng phải có ít nhất 2 đội.`);
        }
        ...
        if (configuredGroups !== undefined && !Array.isArray(configuredGroups)) {
            throw new common_1.BadRequestException(`${sourceLabel}: danh sách cấu hình bảng phải là array.`);
        }
        if (Array.isArray(configuredGroups)) {
            if (numGroups != null && configuredGroups.length !== numGroups) {
                throw new common_1.BadRequestException(`${sourceLabel}: số cấu hình bảng phải khớp numGroups.`);
            }
            for (const [index, configuredGroup] of configuredGroups.entries()) {
                if (!isRecord(configuredGroup)) {
                    throw new common_1.BadRequestException(`${sourceLabel}.groupsConfig.groups.${index}: cấu hình bảng phải là object.`);
                }
            }
        }
    }
    ...
    if (payload.advancementConfig !== undefined && !advancementConfig) {
        throw new common_1.BadRequestException(`${sourceLabel}: advancementConfig phải là object.`);
    }
    if (advancementConfig) {
        ...
        if (allowWildcard !== undefined && typeof allowWildcard !== 'boolean') {
            throw new common_1.BadRequestException(`${sourceLabel}: allowWildcardThird phải là boolean.`);
        }
    }
    const playoffConfig = asRecord(payload.playoffConfig);
    if (payload.playoffConfig !== undefined && !playoffConfig) {
        throw new common_1.BadRequestException(`${sourceLabel}: playoffConfig phải là object.`);
    }
    if (playoffConfig) {
        if (playoffConfig.type !== undefined && !['SINGLE_ELIMINATION', 'DOUBLE_ELIMINATION'].includes(String(playoffConfig.type))) {
            throw new common_1.BadRequestException(`${sourceLabel}: thể thức playoff không hợp lệ.`);
        }
        if (playoffConfig.seedingType !== undefined && !['SEEDED', 'RANDOM'].includes(String(playoffConfig.seedingType))) {
            throw new common_1.BadRequestException(`${sourceLabel}: cách xếp hạt giống playoff không hợp lệ.`);
        }
    }
    const tiebreakerRules = asRecord(payload.tiebreakerRules);
    if (payload.tiebreakerRules !== undefined && !tiebreakerRules) {
        throw new common_1.BadRequestException(`${sourceLabel}: tiebreakerRules phải là object.`);
    }
    if (tiebreakerRules) {
        ...
        if (tiebreakerRules.primary !== undefined && !allowedRules.includes(String(tiebreakerRules.primary))) {
            throw new common_1.BadRequestException(`${sourceLabel}: tiêu chí phân hạng chính không hợp lệ.`);
        }
        if (tiebreakerRules.secondary !== undefined && (!Array.isArray(tiebreakerRules.secondary) ||
            tiebreakerRules.secondary.some((rule) => !allowedRules.includes(String(rule))))) {
            throw new common_1.BadRequestException(`${sourceLabel}: danh sách tiêu chí phân hạng phụ không hợp lệ.`);
        }
    }
}
function validateSportRuleConfig(payload, options) {
    if (payload == null) {
        return;
    }
    if (!isRecord(payload)) {
        throw new common_1.BadRequestException(`${options.sourceLabel}: cấu hình luật phải là object.`);
    }
    const payloadKind = normalizeSportRuleKind(payload.kind);
    const kind = payloadKind ?? options.expectedKind ?? null;
    if (!kind) {
        throw new common_1.BadRequestException(`${options.sourceLabel}: không xác định được kind của luật thi đấu.`);
    }
    ...
    if (allowedKinds.length > 0 && !allowedKinds.includes(kind)) {
        throw new common_1.BadRequestException(`${options.sourceLabel}: kind ${kind} không nằm trong nhóm luật được phép (${allowedKinds.join(', ')}).`);
    }
    const allowedKeys = new Set(COMMON_ROOT_KEYS);
    ...
    for (const key of Object.keys(payload)) {
        if (!allowedKeys.has(key)) {
            throw new common_1.BadRequestException(`${options.sourceLabel}: field ${key} không hợp lệ cho luật ${kind}.`);
        }
    }
    ...
    if (scoringModel !== undefined && scoringModel !== SCORING_MODEL_BY_KIND[kind]) {
        throw new common_1.BadRequestException(`${options.sourceLabel}: scoringModel ${String(scoringModel)} không khớp với kind ${kind}.`);
    }
        ...
        if (rounds) {
            for (const [roundKey, roundValue] of Object.entries(rounds)) {
                if (!isRecord(roundValue)) {
                    throw new common_1.BadRequestException(`${options.sourceLabel}.rounds.${roundKey}: cấu hình vòng phải là object.`);
                }
...
function inferExpectedSportRuleKind(source) {
    return inferSportRuleKindFromCategory(source);
}
function inferAllowedSportRuleKinds(source) {
    return resolveAllowedKindsFromCategory(source);
}
//# sourceMappingURL=validate-sport-rules-config.js.map