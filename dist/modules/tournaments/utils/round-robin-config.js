...
exports.asConfigRecord = asConfigRecord;
exports.extractSportRuleOverrides = extractSportRuleOverrides;
exports.resolveConfiguredGroups = resolveConfiguredGroups;
exports.resolveRoundsToPlay = resolveRoundsToPlay;
exports.resolveRoundRobinGroupCount = resolveRoundRobinGroupCount;
exports.buildRoundRobinSchedule = buildRoundRobinSchedule;
exports.allocateRoundRobinGroups = allocateRoundRobinGroups;
...
function extractSportRuleOverrides(value) {
    ...
    for (const nestedKey of ['sportRules', 'roundConfig', 'matchRules', 'rules']) {
        ...
        if (nested)
            Object.assign(overrides, extractSportRuleOverrides(nested));
    }
    return overrides;
}
function findConfiguredGroupArray(source) {
...
function resolveConfiguredGroups(...sources) {
...
function resolveRoundsToPlay(...sources) {
    for (const source of sources) {
        if (!source)
            continue;
        const groupsConfig = asConfigRecord(source.groupsConfig);
...
function resolveRoundRobinGroupCount(groupsConfig, configuredGroups, participantCount, maxPerGroup) {
...
function allocateRoundRobinGroups(participants, groupCount, configuredGroups, maxPerGroup) {
    ...
    configuredGroups.forEach((group, groupIndex) => {
        ...
        for (const participantId of group.participantIds) {
            ...
            if (!participant)
                throw new Error(`Đội ${participantId} không thuộc danh sách đủ điều kiện.`);
            if (assignedIds.has(participantId))
                throw new Error(`Đội ${participantId} được cấu hình ở nhiều bảng.`);
    ...
    for (const participant of remaining) {
        ...
        if (direction === 1 && cursor === groupCount - 1)
            direction = -1;
...
//# sourceMappingURL=round-robin-config.js.map