...
exports.hasFootballScoreSnapshot = hasFootballScoreSnapshot;
exports.sortFootballStandings = sortFootballStandings;
function hasFootballScoreSnapshot(matches) {
...
function sortFootballStandings(rows, matches) {
    const participantIds = new Set(rows.map((row) => row.participantId));
    const headToHead = new Map(rows.map((row) => [row.participantId, 0]));
    const fairPlay = new Map(rows.map((row) => [row.participantId, 0]));
    const groupId = rows[0]?.groupId;
    for (const match of matches) {
        if (match.groupId !== groupId || !match.participant1Id || !match.participant2Id ||
            !participantIds.has(match.participant1Id) || !participantIds.has(match.participant2Id))
            continue;
        ...
        const football = match.scoreDetails && typeof match.scoreDetails === 'object'
            ? match.scoreDetails.football
            : null;
        const events = football && typeof football === 'object' && Array.isArray(football.events)
            ? football.events
            : [];
        for (const event of events) {
            ...
            if (target)
                fairPlay.set(target, (fairPlay.get(target) || 0) + penalty);
        }
    }
...
//# sourceMappingURL=football-standings.js.map