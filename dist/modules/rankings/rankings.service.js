...
const ELO_SHIELD_BOUNDARIES = [1100, 1200, 1300, 1400, 1500, 1600, 1700, 1800];
...
let RankingsService = class RankingsService {
    ...
    constructor(db, rankingsRepository, eloEngineService, redisService, footballTeamEloService) {
        this.db = db;
        this.rankingsRepository = rankingsRepository;
        this.eloEngineService = eloEngineService;
        this.redisService = redisService;
        this.footballTeamEloService = footballTeamEloService;
    }
    async invalidateLeaderboardCache(categoryId) {
        try {
    ...
    extractScoreRatio(scoreDetails, winnerParticipantId, participant1Id) {
    ...
    async updateMatchElo(dto) {
        ...
        const [matchContext] = await db
            ...
            .where((0, drizzle_orm_1.eq)(schema.matches.id, dto.matchId))
            .limit(1);
        if (!matchContext) {
            throw new common_1.BadRequestException('Trận đấu không tồn tại.');
        }
        if (matchContext.status !== 'COMPLETED') {
            throw new common_1.BadRequestException('Chỉ trận đã hoàn tất mới được tính ELO.');
        }
        if (!matchContext.isRanked) {
            throw new common_1.BadRequestException('Trận đấu thuộc giải không xếp hạng ELO.');
        }
        ...
        if (effectiveMatchType === 'DOUBLES' || effectiveMatchType === 'MIXED_DOUBLES') {
            throw new common_1.BadRequestException('Trận đôi phải được tính ELO qua luồng hoàn tất trận, không dùng endpoint ELO thủ công cho user đơn.');
        }
        ...
        if (matchContext.tournamentType === 'CLUB' && !effectiveCommunityId) {
            throw new common_1.BadRequestException('Giải CLB phải có câu lạc bộ để ghi ELO nội bộ.');
        }
        if (dto.categoryId !== matchContext.categoryId ||
            ...
            throw new common_1.BadRequestException('Thông tin tính ELO không khớp cấu hình của trận đấu.');
        }
        ...
        try {
            const [matchData] = await db
                ...
                .limit(1);
            if (matchData) {
                scoreRatio = this.extractScoreRatio(matchData.scoreDetails, dto.winnerId, matchData.participant1Id);
            }
        }
        catch (err) {
        ...
        const result = await db.transaction(async (tx) => {
            await tx.execute((0, drizzle_orm_1.sql) `select pg_advisory_xact_lock(hashtext(${dto.matchId}))`);
            const previousLogs = await tx
                .select({ userId: schema.eloHistoryLogs.userId })
                .from(schema.eloHistoryLogs)
                .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema.eloHistoryLogs.matchId, dto.matchId), (0, drizzle_orm_1.inArray)(schema.eloHistoryLogs.userId, [dto.winnerId, dto.loserId])));
            if (previousLogs.length > 0) {
                if (previousLogs.length === 2) {
                    return { alreadyProcessed: true, matchId: dto.matchId };
                }
                throw new common_1.BadRequestException('Lịch sử ELO của trận đấu không đầy đủ; không thể tính lại tự động.');
            }
            const winnerRank = await this.rankingsRepository.getOrCreateUserRank(tx, dto.winnerId, dto.categoryId, dto.matchType, scope, dto.communityId, true, dto.genderRestriction);
            const loserRank = await this.rankingsRepository.getOrCreateUserRank(tx, dto.loserId, dto.categoryId, dto.matchType, scope, dto.communityId, true, dto.genderRestriction);
            const winnerResult = this.eloEngineService.calculateElo(winnerRank.eloPoints, loserRank.eloPoints, true, winnerRank.matchesPlayed, winnerRank.winStreak, scoreRatio);
            const loserResult = this.eloEngineService.calculateElo(loserRank.eloPoints, winnerRank.eloPoints, false, loserRank.matchesPlayed, loserRank.winStreak, scoreRatio);
            let isWinnerShieldActive = false;
            if (scope === 'PUBLIC') {
                const publicWinnerRank = winnerRank;
            ...
            if (scope === 'PUBLIC') {
                const publicLoserRank = loserRank;
            ...
            await this.rankingsRepository.updateUserRank(tx, winnerRank.id, {
            ...
            await this.rankingsRepository.updateUserRank(tx, loserRank.id, {
    ...
    async processMatchResult(matchId, winnerParticipantId, loserParticipantId, categoryId, matchType, scope, communityId, genderRestriction) {
        const db = this.rankingsRepository.getDbInstance();
        const winnerRosters = await db
            ...
            .where((0, drizzle_orm_1.eq)(schema.tournamentRosters.participantId, winnerParticipantId));
        const loserRosters = await db
            ...
            .innerJoin(schema.users, (0, drizzle_orm_1.eq)(schema.tournamentRosters.userId, schema.users.id))
            .innerJoin(schema.tournamentParticipants, (0, drizzle_orm_1.eq)(schema.tournamentRosters.participantId, schema.tournamentParticipants.id))
            .where((0, drizzle_orm_1.eq)(schema.tournamentRosters.participantId, loserParticipantId));
        if (winnerRosters.length === 0 || loserRosters.length === 0) {
            throw new common_1.BadRequestException('Winner or Loser team has no players registered.');
        }
        const hasMockPlayer = [...winnerRosters, ...loserRosters].some((roster) => roster.userIsMock || roster.participantIsMock);
        if (hasMockPlayer) {
            return {
                success: true,
                skipped: true,
        ...
        if (winnerUserIds.length !== expectedRosterSize || loserUserIds.length !== expectedRosterSize) {
            throw new common_1.BadRequestException(`${isDoublesMatch ? 'Trận đôi' : 'Trận đơn'} phải có đúng ${expectedRosterSize} vận động viên mỗi bên để tính ELO.`);
        }
        let scoreRatio;
        try {
            const [matchData] = await db
                ...
                .limit(1);
            if (matchData) {
                scoreRatio = this.extractScoreRatio(matchData.scoreDetails, winnerParticipantId, matchData.participant1Id);
            }
        }
        catch (err) {
        ...
        const result = await db.transaction(async (tx) => {
            await tx.execute((0, drizzle_orm_1.sql) `select pg_advisory_xact_lock(hashtext(${matchId}))`);
            const previousLogs = await tx
                .select({ userId: schema.eloHistoryLogs.userId })
                .from(schema.eloHistoryLogs)
                .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema.eloHistoryLogs.matchId, matchId), (0, drizzle_orm_1.inArray)(schema.eloHistoryLogs.userId, [...winnerUserIds, ...loserUserIds])));
            if (previousLogs.length > 0) {
            ...
            if (isDoublesMatch) {
                ...
                let [winnerPair] = await tx
                    ...
                    .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema.pairRanks.user1Id, wId1), (0, drizzle_orm_1.eq)(schema.pairRanks.user2Id, wId2), (0, drizzle_orm_1.eq)(schema.pairRanks.categoryId, categoryId), (0, drizzle_orm_1.eq)(schema.pairRanks.matchType, matchType), (0, drizzle_orm_1.eq)(schema.pairRanks.scope, scope), genderRestriction
                    ? (0, drizzle_orm_1.eq)(schema.pairRanks.genderRestriction, genderRestriction)
                    : (0, drizzle_orm_1.isNull)(schema.pairRanks.genderRestriction), communityId
                    ? (0, drizzle_orm_1.eq)(schema.pairRanks.communityId, communityId)
                    : (0, drizzle_orm_1.isNull)(schema.pairRanks.communityId)))
                    .limit(1);
                if (!winnerPair) {
                    [winnerPair] = await tx
                ...
                let [loserPair] = await tx
                    .select()
                    .from(schema.pairRanks)
                    .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema.pairRanks.user1Id, lId1), (0, drizzle_orm_1.eq)(schema.pairRanks.user2Id, lId2), (0, drizzle_orm_1.eq)(schema.pairRanks.categoryId, categoryId), (0, drizzle_orm_1.eq)(schema.pairRanks.matchType, matchType), (0, drizzle_orm_1.eq)(schema.pairRanks.scope, scope), genderRestriction
                    ? (0, drizzle_orm_1.eq)(schema.pairRanks.genderRestriction, genderRestriction)
                    : (0, drizzle_orm_1.isNull)(schema.pairRanks.genderRestriction), communityId
                    ? (0, drizzle_orm_1.eq)(schema.pairRanks.communityId, communityId)
                    : (0, drizzle_orm_1.isNull)(schema.pairRanks.communityId)))
                ...
                const winnerPairResult = this.eloEngineService.calculateElo(winnerPair.eloPoints, loserPair.eloPoints, true, winnerPair.matchesPlayed, winnerPair.winStreak, scoreRatio);
                const loserPairResult = this.eloEngineService.calculateElo(loserPair.eloPoints, winnerPair.eloPoints, false, loserPair.matchesPlayed, loserPair.winStreak, scoreRatio);
                ...
                await tx
                    ...
                    .where((0, drizzle_orm_1.eq)(schema.pairRanks.id, winnerPair.id));
                await tx
                    .update(schema.pairRanks)
                    .set({
                    ...
                    .where((0, drizzle_orm_1.eq)(schema.pairRanks.id, loserPair.id));
                ...
                for (const { rank, delta } of winnersToUpdate) {
                    ...
                    let isWinnerShieldActive = false;
                    if (scope === 'PUBLIC') {
                        isWinnerShieldActive = !!rank.shieldActive;
                        for (const boundary of ELO_SHIELD_BOUNDARIES) {
                    ...
                    await this.rankingsRepository.updateUserRank(tx, rank.id, {
                        eloPoints: newElo,
                ...
                if (scope === 'PUBLIC') {
                    for (const uid of [...winnerUserIds, ...loserUserIds]) {
                        await tx
                            ...
                            .set({ lastActiveAt: dNow, lastDecayAt: dNow })
                            .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema.userRanks.userId, uid), (0, drizzle_orm_1.eq)(schema.userRanks.categoryId, categoryId), (0, drizzle_orm_1.eq)(schema.userRanks.matchType, matchType), genderRestriction
                            ? (0, drizzle_orm_1.eq)(schema.userRanks.genderRestriction, genderRestriction)
                            : (0, drizzle_orm_1.isNull)(schema.userRanks.genderRestriction), (0, drizzle_orm_1.isNull)(schema.userRanks.communityId)));
                    }
                }
                else if (scope === 'COMMUNITY') {
                    for (const uid of [...winnerUserIds, ...loserUserIds]) {
                        await tx
                            ...
                            .set({ lastActiveAt: dNow, lastDecayAt: dNow })
                            .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema.communityRankings.userId, uid), (0, drizzle_orm_1.eq)(schema.communityRankings.categoryId, categoryId), (0, drizzle_orm_1.eq)(schema.communityRankings.matchType, matchType), genderRestriction
                            ? (0, drizzle_orm_1.eq)(schema.communityRankings.genderRestriction, genderRestriction)
                            : (0, drizzle_orm_1.isNull)(schema.communityRankings.genderRestriction), communityId ? (0, drizzle_orm_1.eq)(schema.communityRankings.communityId, communityId) : undefined));
                    }
                }
                return {
                    success: true,
            ...
            for (const userId of winnerUserIds) {
                const rank = await this.rankingsRepository.getOrCreateUserRank(tx, userId, categoryId, matchType, scope, communityId, true, genderRestriction);
            ...
            for (const rank of winnerRanks) {
                const result = this.eloEngineService.calculateElo(rank.eloPoints, avgLoserElo, true, rank.matchesPlayed, rank.winStreak, scoreRatio);
                let isWinnerShieldActive = false;
                if (scope === 'PUBLIC') {
                    isWinnerShieldActive = !!rank.shieldActive;
                    for (const boundary of ELO_SHIELD_BOUNDARIES) {
            ...
            for (const rank of loserRanks) {
                const result = this.eloEngineService.calculateElo(rank.eloPoints, avgWinnerElo, false, rank.matchesPlayed, rank.winStreak, scoreRatio);
            ...
            for (const userId of [...winnerUserIds, ...loserUserIds]) {
                if (scope === 'COMMUNITY') {
                    await tx
                        .update(schema.communityRankings)
                        .set({ lastActiveAt: now, lastDecayAt: now })
                        .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema.communityRankings.userId, userId), (0, drizzle_orm_1.eq)(schema.communityRankings.categoryId, categoryId), (0, drizzle_orm_1.eq)(schema.communityRankings.matchType, matchType), genderRestriction
                        ? (0, drizzle_orm_1.eq)(schema.communityRankings.genderRestriction, genderRestriction)
                        : (0, drizzle_orm_1.isNull)(schema.communityRankings.genderRestriction), communityId ? (0, drizzle_orm_1.eq)(schema.communityRankings.communityId, communityId) : undefined));
                }
                else {
                    await tx
                        .update(schema.userRanks)
                        .set({ lastActiveAt: now, lastDecayAt: now })
                        .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema.userRanks.userId, userId), (0, drizzle_orm_1.eq)(schema.userRanks.categoryId, categoryId), (0, drizzle_orm_1.eq)(schema.userRanks.matchType, matchType), genderRestriction
                        ? (0, drizzle_orm_1.eq)(schema.userRanks.genderRestriction, genderRestriction)
                        : (0, drizzle_orm_1.isNull)(schema.userRanks.genderRestriction), (0, drizzle_orm_1.isNull)(schema.userRanks.communityId)));
                }
            }
    ...
    async recalculateUserRankTier(tx, userId, categoryId, matchType, genderRestriction) {
        ...
        const [topRank] = await tx
            ...
            .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema.userRanks.categoryId, categoryId), (0, drizzle_orm_1.eq)(schema.userRanks.matchType, matchType), genderCondition, (0, drizzle_orm_1.isNull)(schema.userRanks.communityId)))
            .orderBy((0, drizzle_orm_1.desc)(schema.userRanks.eloPoints), schema.userRanks.updatedAt)
            .limit(1);
        ...
        if (tierS) {
            if (tierSUserId) {
                await tx
                    .update(schema.userRanks)
                    .set({ tierId: tierS.id })
                    .where((0, drizzle_orm_1.eq)(schema.userRanks.id, topRank.id));
                await tx
                    .update(schema.userRanks)
                    .set({ tierId: tierAHigh ? tierAHigh.id : null })
                    .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema.userRanks.categoryId, categoryId), (0, drizzle_orm_1.eq)(schema.userRanks.matchType, matchType), genderCondition, (0, drizzle_orm_1.isNull)(schema.userRanks.communityId), (0, drizzle_orm_1.eq)(schema.userRanks.tierId, tierS.id), (0, drizzle_orm_1.sql) `${schema.userRanks.userId} != ${tierSUserId}`));
            }
            else {
                await tx
                    .update(schema.userRanks)
                    .set({ tierId: tierAHigh ? tierAHigh.id : null })
                    .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema.userRanks.categoryId, categoryId), (0, drizzle_orm_1.eq)(schema.userRanks.matchType, matchType), genderCondition, (0, drizzle_orm_1.isNull)(schema.userRanks.communityId), (0, drizzle_orm_1.eq)(schema.userRanks.tierId, tierS.id)));
            }
        }
        if (userId !== tierSUserId) {
            const [rank] = await tx
                .select()
                .from(schema.userRanks)
                .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema.userRanks.userId, userId), (0, drizzle_orm_1.eq)(schema.userRanks.categoryId, categoryId), (0, drizzle_orm_1.eq)(schema.userRanks.matchType, matchType), genderCondition, (0, drizzle_orm_1.isNull)(schema.userRanks.communityId)))
                .limit(1);
            if (rank) {
                const elo = rank.eloPoints;
                let targetTier = null;
                if (elo >= 1700 && tierAHigh)
                    targetTier = tierAHigh;
                else if (elo >= 1600 && tierALow)
                    targetTier = tierALow;
                else if (elo >= 1500 && tierBHigh)
                    targetTier = tierBHigh;
                else if (elo >= 1400 && tierBLow)
                    targetTier = tierBLow;
                else if (elo >= 1300 && tierCHigh)
                    targetTier = tierCHigh;
                else if (elo >= 1200 && tierCLow)
                    targetTier = tierCLow;
                else if (elo >= 1100 && tierDHigh)
                    targetTier = tierDHigh;
                else if (tierDLow)
                    targetTier = tierDLow;
                await tx
                    .update(schema.userRanks)
                    .set({ tierId: targetTier ? targetTier.id : null })
                    .where((0, drizzle_orm_1.eq)(schema.userRanks.id, rank.id));
            }
        }
    }
    async applyMonthlyInactivityDecay() {
        const now = new Date();
        const inactiveBefore = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth() - ELO_DECAY_INACTIVE_MONTHS, now.getUTCDate(), now.getUTCHours(), now.getUTCMinutes(), now.getUTCSeconds()));
        const currentMonthStart = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), 1));
        const affectedCategoryIds = new Set();
        await this.db.transaction(async (tx) => {
            const lockResult = await tx.execute((0, drizzle_orm_1.sql) `select pg_try_advisory_xact_lock(hashtext('elo-monthly-inactivity-decay')) as locked`);
            ...
            const publicRanks = await tx
                .select()
                .from(schema.userRanks)
                .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.lt)(schema.userRanks.lastActiveAt, inactiveBefore), (0, drizzle_orm_1.lt)(schema.userRanks.lastDecayAt, currentMonthStart), (0, drizzle_orm_1.isNull)(schema.userRanks.communityId)));
            const communityRanks = await tx
                .select()
                .from(schema.communityRankings)
                .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.lt)(schema.communityRankings.lastActiveAt, inactiveBefore), (0, drizzle_orm_1.lt)(schema.communityRankings.lastDecayAt, currentMonthStart)));
            const pairRanks = await tx
                .select()
                .from(schema.pairRanks)
                .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.lt)(schema.pairRanks.lastActiveAt, inactiveBefore), (0, drizzle_orm_1.lt)(schema.pairRanks.lastDecayAt, currentMonthStart)));
            const monthsSince = (lastDecayAt) => Math.max(1, (now.getUTCFullYear() - lastDecayAt.getUTCFullYear()) * 12 +
                now.getUTCMonth() - lastDecayAt.getUTCMonth());
            const decayedElo = (elo, months) => {
                if (elo < ELO_DECAY_THRESHOLD)
                    return elo;
                const rate = ELO_DECAY_RATES.find((bracket) => elo >= bracket.minElo)?.rate;
                if (!rate)
                    return elo;
                return Math.max(ELO_DECAY_FLOOR, Math.round(elo * Math.pow(1 - rate, months)));
            };
            ...
            for (const rank of communityRanks) {
                ...
                await tx.update(schema.communityRankings)
                    ...
                    .where((0, drizzle_orm_1.eq)(schema.communityRankings.id, rank.id));
            ...
            for (const rank of pairRanks) {
                ...
                await tx.update(schema.pairRanks)
                    .set({ eloPoints: newElo, lastDecayAt: now, updatedAt: now })
                    .where((0, drizzle_orm_1.eq)(schema.pairRanks.id, rank.id));
                affectedCategoryIds.add(rank.categoryId);
            }
        });
        await Promise.all([...affectedCategoryIds].map((categoryId) => this.invalidateLeaderboardCache(categoryId)));
    }
    async recalculateCommunityRankTier(tx, userId, categoryId, matchType, communityId, genderRestriction) {
        const tiers = await tx
            .select()
            .from(schema.eloTiers)
            .where((0, drizzle_orm_1.eq)(schema.eloTiers.categoryId, categoryId))
        ...
        const [rank] = await tx
            .select()
            .from(schema.communityRankings)
            .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema.communityRankings.userId, userId), (0, drizzle_orm_1.eq)(schema.communityRankings.categoryId, categoryId), (0, drizzle_orm_1.eq)(schema.communityRankings.matchType, matchType), genderCondition, (0, drizzle_orm_1.eq)(schema.communityRankings.communityId, communityId ?? '')))
            .limit(1);
        if (rank) {
            const elo = rank.eloPoints;
            let targetTier = null;
    ...
    async recalculateUserTiersOnProvinceChange(userId) {
        const db = this.rankingsRepository.getDbInstance();
        ...
        await db.transaction(async (tx) => {
            for (const rank of ranks) {
                await this.recalculateUserRankTier(tx, userId, rank.categoryId, rank.matchType, rank.genderRestriction || undefined);
            }
        });
    }
    async recalculateEloChain(tx, playerIds, fromTime, categoryId, matchType) {
        const subsequentMatches = await tx
            ...
            .innerJoin(schema.tournamentRosters, (0, drizzle_orm_1.or)((0, drizzle_orm_1.eq)(schema.matches.participant1Id, schema.tournamentRosters.participantId), (0, drizzle_orm_1.eq)(schema.matches.participant2Id, schema.tournamentRosters.participantId)))
            .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema.tournaments.categoryId, categoryId), (0, drizzle_orm_1.eq)(schema.tournaments.matchType, matchType), (0, drizzle_orm_1.eq)(schema.matches.status, 'COMPLETED'), (0, drizzle_orm_1.gte)(schema.matches.completedAt, fromTime)))
            .orderBy((0, drizzle_orm_1.asc)(schema.matches.completedAt));
        const uniqueMatchIds = new Set();
        const matchesToRecalculate = [];
        ...
        const affectedPlayers = new Set(playerIds);
        const playerStates = new Map();
        const getPlayerState = async (userId, completedAt) => {
            ...
            const [lastLog] = await tx
                .select()
                .from(schema.eloHistoryLogs)
                .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema.eloHistoryLogs.userId, userId), (0, drizzle_orm_1.eq)(schema.eloHistoryLogs.categoryId, categoryId), (0, drizzle_orm_1.lt)(schema.eloHistoryLogs.createdAt, completedAt)))
                .orderBy((0, drizzle_orm_1.desc)(schema.eloHistoryLogs.createdAt))
                .limit(1);
            ...
            const playedRes = await tx
                ...
                .innerJoin(schema.tournaments, (0, drizzle_orm_1.eq)(schema.tournamentStages.tournamentId, schema.tournaments.id))
                .innerJoin(schema.tournamentRosters, (0, drizzle_orm_1.or)((0, drizzle_orm_1.eq)(schema.matches.participant1Id, schema.tournamentRosters.participantId), (0, drizzle_orm_1.eq)(schema.matches.participant2Id, schema.tournamentRosters.participantId)))
                .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema.tournamentRosters.userId, userId), (0, drizzle_orm_1.eq)(schema.tournaments.categoryId, categoryId), (0, drizzle_orm_1.eq)(schema.tournaments.matchType, matchType), (0, drizzle_orm_1.eq)(schema.matches.status, 'COMPLETED'), (0, drizzle_orm_1.lt)(schema.matches.completedAt, completedAt)));
            ...
            const wonRes = await tx
                ...
                .innerJoin(schema.tournamentStages, (0, drizzle_orm_1.eq)(schema.tournamentGroups.stageId, schema.tournamentStages.id))
                .innerJoin(schema.tournaments, (0, drizzle_orm_1.eq)(schema.tournamentStages.tournamentId, schema.tournaments.id))
                .innerJoin(schema.tournamentRosters, (0, drizzle_orm_1.eq)(schema.matches.winnerId, schema.tournamentRosters.participantId))
                .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema.tournamentRosters.userId, userId), (0, drizzle_orm_1.eq)(schema.tournaments.categoryId, categoryId), (0, drizzle_orm_1.eq)(schema.tournaments.matchType, matchType), (0, drizzle_orm_1.eq)(schema.matches.status, 'COMPLETED'), (0, drizzle_orm_1.lt)(schema.matches.completedAt, completedAt)));
        ...
        for (const item of matchesToRecalculate) {
            ...
            const matchScoreRatio = match.scoreDetails
                ? this.extractScoreRatio(match.scoreDetails, winnerParticipantId, match.participant1Id)
                : undefined;
            for (let i = 0; i < winnerUserIds.length; i++) {
                const uid = winnerUserIds[i];
                const state = winnerStates[i];
                const result = this.eloEngineService.calculateElo(state.elo, avgLoserElo, true, state.matchesPlayed, state.winStreak, matchScoreRatio);
                const boundaries = [1100, 1200, 1300, 1500, 1700, 1800];
                ...
                await tx
                    ...
                    .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema.eloHistoryLogs.matchId, match.id), (0, drizzle_orm_1.eq)(schema.eloHistoryLogs.userId, uid)));
            ...
            for (let i = 0; i < loserUserIds.length; i++) {
                ...
                const result = this.eloEngineService.calculateElo(state.elo, avgWinnerElo, false, state.matchesPlayed, state.winStreak, matchScoreRatio);
                const boundaries = [1100, 1200, 1300, 1500, 1700, 1800];
        ...
        for (const [uid, state] of playerStates.entries()) {
            await tx
                ...
                .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema.userRanks.userId, uid), (0, drizzle_orm_1.eq)(schema.userRanks.categoryId, categoryId), (0, drizzle_orm_1.eq)(schema.userRanks.matchType, matchType), (0, drizzle_orm_1.isNull)(schema.userRanks.communityId)));
            await this.recalculateUserRankTier(tx, uid, categoryId, matchType);
        }
        await this.invalidateLeaderboardCache(categoryId);
    }
    async recalculateEloChainSafe(playerIds, fromTime, categoryId, matchType) {
        await this.db.transaction(async (tx) => {
            await this.recalculateEloChain(tx, playerIds, fromTime, categoryId, matchType);
        });
    }
    async processMatchResultFromOutbox(matchId) {
        const footballResult = await this.footballTeamEloService?.processCompletedMatch(matchId);
        if (footballResult?.handled)
        ...
        const scope = tournament.tournamentType === 'CLUB' && tournament.communityId ? 'COMMUNITY' : 'PUBLIC';
        return this.processMatchResult(matchId, match.winnerId, loserId, tournament.categoryId, tournament.matchType, scope, tournament.communityId || undefined, tournament.genderRestriction || undefined);
    }
};
...
], RankingsService.prototype, "applyMonthlyInactivityDecay", null);
exports.RankingsService = RankingsService = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, common_1.Inject)(database_module_1.PG_CONNECTION)),
    __param(4, (0, common_1.Optional)()),
    __metadata("design:paramtypes", [Object, rankings_repository_1.RankingsRepository,
        elo_engine_service_1.EloEngineService,
